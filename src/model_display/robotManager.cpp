...
void RobotManager::initialize()
{
    ...
    if (jointConfigPath.isEmpty()) {
        // 如果 resolveConfigPath 找不到，尝试更多路径
        QStringList candidatePaths = {
            "../config/boneToJoint.csv",
            "../../config/boneToJoint.csv",
            "../../RobanWeb-main/config/boneToJoint.csv",
            "config/boneToJoint.csv",
            "../RobanWeb-main/config/boneToJoint.csv"
        };
        for (const QString &path : candidatePaths) {
            if (QFile::exists(path)) {
                jointConfigPath = QDir::cleanPath(path);
                qDebug() << "RobotManager::initialize: found boneToJoint.csv at" << jointConfigPath;
                break;
            }
        }
    }
    if (!jointConfigPath.isEmpty()) {
        ...
        qWarning() << "RobotManager::initialize: boneToJoint.csv not found! Joint animation will not work.";
        qWarning() << "  Searched in:" << QCoreApplication::applicationDirPath() << "and" << QDir::currentPath();
    }
